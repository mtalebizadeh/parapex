write_param_basin <- function(uncertain_watershed_param,
                               param_file.dat,
                               Back_Up_param_file.dat) {
  #Writes parameter values inside APEX PARM file.
  #
  # Args:
  #   uncertain_watershed_param: dataframe containing APEX basin parameters.
  #   param_file.dat: PARM file name.
  #   Back_Up_param_file.dat: PARM file name containing default APEX parameters.
  #
  # Returns:
  #   void (writes sampled parameter values inside the PARM file).
  #
  conn_source <- file(description = Back_Up_param_file.dat,open = "r+",
                      blocking = TRUE)
  conn_target <- file(description = param_file.dat,open ="r+",
                      blocking = TRUE )
  #Reading from back up file into a readLines object...
  readlines_obj <- readLines(con = conn_source,n = -1)
  i = 1
  col_numbers <-ncol(uncertain_watershed_param)
  while (i<=col_numbers) {
    readlines_obj <- parm2replace(readlines_obj,uncertain_watershed_param[i])
    i= i + 1
  }
...
#### Sub-Function....
    parm2replace <- function(readlines_obj,PARM) {
